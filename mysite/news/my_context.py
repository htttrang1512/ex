...
def items_article_footer_random (request):
    ...
    items_article_footer_random = Article.objects.filter(status= APP_VALUE_STATUS_ACTIVE, publish_date__lte= timezone.now()).exclude(slug=skip_slug).order_by('?')[:SETTING_ARTICLE_TOTAL_ITEMS_RELATED][:SETTING_ARTICLE_TOTAL_ITEMS_RANDOM]
    return {'items_article_footer_random' : items_article_footer_random}

def items_article_header_trending (request):
    skip_slug = request.get_full_path().replace("/article/", "")

    items_article_header_trending = Article.objects.filter(status= APP_VALUE_STATUS_ACTIVE, publish_date__lte= timezone.now()).exclude(slug=skip_slug).order_by('?')[:SETTING_ARTICLE_TOTAL_ITEMS_RELATED][:3]
